...
public class StyleMatiereManager extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet StyleMatiereManager</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet StyleMatiereManager at " + request.getContextPath() + "</h1>");
            String style=request.getParameter("style");
            ConnectionBase base=new ConnectionBase();
            ArrayList<Matiere> matieres=Matiere.getAllByStyle(base.giveConSql(), Integer.valueOf(style));
            Style tyle=Style.getStyleById(base.giveConSql(), Integer.valueOf(style));
            tyle.setMatierePrim(matieres);
            out.println("</body>");
